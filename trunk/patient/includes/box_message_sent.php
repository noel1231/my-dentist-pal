...
$pt_id=$_SESSION['patient_id'];
 function pagination($query, $per_page = 10,$page = 1, $url = '?'){        
    	$query = "SELECT COUNT(*) as num FROM $query";			$query1=mysql_query($query);
...
$sql=mysql_query("SELECT dentist_id FROM patient_list WHERE id='".$pt_id."'");
...
if(isset($_POST['reply']))
{
	...
	if(count($_POST["check"]) >= 2) {
		...
		if($_POST["check"][$i] != "") {
			$qwerty="SELECT * FROM patient_message_sent WHERE id='".$_POST['check'][$i]."'";
...
$rows=$query['num'];?>
    	...
    	$startpoint = ($page * $limit) - $limit;
		 //$statement = "`records` where `active` = 1";
			  if(isset($_GET['arr']))
			   ...
			   $query="SELECT * FROM patient_message_sent WHERE patient_id='".$pt_id."' ORDER by $sort $arrange LIMIT {$startpoint} , {$limit}";
			   $res=mysql_query($query); 
			   
			     $statement = "patient_message_sent WHERE patient_id='".$pt_id."' ORDER by $sort $arrange";
			   ...
			   $query="SELECT * FROM patient_message_sent WHERE message_subject LIKE '%".$name."%' AND patient_id='".$pt_id."'";
			   $res=mysql_query($query);
			   }
			   //search
			  ...
			  $sql="SELECT * FROM patient_message_sent WHERE patient_id='".$pt_id."' LIMIT {$startpoint} , {$limit}";
			  $res=mysql_query($sql);
	...
	$query=mysql_query("UPDATE patient_message_sent SET dentist_name='".$name_now."' WHERE id='".$msg_id."'");
              ...
              <!--check--></td>
              <!--name--><td style="width:240px;padding-left:10px;padding-top:10px;padding-bottom:10px;text-align: left;">
              <a href="read_message.php?key=<?php echo $row["id"];?>" style="text-decoration:none;color:#373838;"><div><?php 
			   echo "Dr. $name_now";?></div></a>
              <!--name--></td>
                <!--subject--><td style="width:228px;padding-top:10px;padding-bottom:10px;text-align: left;padding-left:10px;">
              <div><?php echo $row["message_subject"];?></div>
              <!--subject--></td>
              <!--last visit--><td style="width:142px;padding-top:10px;padding-bottom:10px;text-align: center;">
              <div><?php echo $row["message_date"];?></div>
              <!--last visit--></td>
              <?php /*<!--action--><td width="108" style="padding-top:10px;padding-bottom:10px;">
             <table cellpadding="0" cellspacing="0" border="0">
             <tr>
             <!--read-->
             <!--print--><td width="25" style="padding-left:20px;"><a href="read_message.php?key=<?php echo $row['id'];?>" title="Read email"><img src="../img/icon_address_send.png" width="18" height="16"/></a></td>
