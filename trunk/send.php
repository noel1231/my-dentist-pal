...
*/
	...
	$mave=$to;
		...
		for($i=0;$i<=$ctr;$i++) {
			...
			$go=$new[$i];
		//var_dump($go);die();
				if(mail($go,$subject,$msg,$headers))
	{
		
$maverick=mysql_query("SELECT * FROM addressbook WHERE email='".$go."' AND dentist_id='".$dentist_id."'");
while($bersabe=mysql_fetch_array($maverick)) {
	$diane=$bersabe['id'];
	$l=$bersabe['last_name'];
	$f=$bersabe['first_name'];
	$myli=$l.", ".$f;
		
	$sql="INSERT INTO message_sent (patient_id,subject_message,date_message,to_message,body_message,patient_name,dentist_id) VALUES('$diane','$subject',NOW(),'$to','$body','$myli','$dentist_id')";
	$res=mysql_query($sql);
	
	$sql="INSERT INTO patient_message_received (patient_id,dentist_id,subject_message,date_message,to_message,body_message,patient_name) VALUES('$diane','$dentist_id','$subject',NOW(),'$to','$body','$myli')";
	$res=mysql_query($sql);
	} }
