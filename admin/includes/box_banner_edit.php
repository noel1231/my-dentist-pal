...
$date_to_activate=$row['date_to_activate'];
$date_to_end=$row['date_to_end'];
$banner_name=$row['banner_name'];
$banner_position=$row['banner_position'];
}

$date=explode("-",$date_to_activate);
$date_year=$date[0];
$date_month=$date[1];
$date_day=$date[2];

$date_to_activate=$date_month."/".$date_day."/".$date_year;

$dates=explode("-",$date_to_end);
$date_y=$dates[0];
$date_m=$dates[1];
$date_d=$dates[2];

$date_to_end=$date_m."/".$date_d."/".$date_y;
...
if(isset($_POST['save']))
{
$id=mysql_real_escape_string($_POST['banner_id']);	
...
$sql="UPDATE banner SET banner_type='".$ban_type."',date_to_activate='".$date_activate."',date_to_end='".$date_end."',banner_name='".$banner_name."',banner_position='".$ban_pos."' WHERE id='".$id."'";
$res=mysql_query($sql);
//var_dump($res);die();
...

   {
	...
	{ 
...

     {
      ...
      $sql = "UPDATE banner SET banner_file='".$uploadfile."' WHERE id='".$id."'";//
      ...
      <tr>
        <td colspan="2" valign="top" style="background:url(../img/3.jpg); padding-bottom:10px;"><div style="margin-top:-8px; position: relative;">
          <table width="690" border="0" align="center" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif;font-size:14px;color:#333;">
            ...
            <select name="banner_type" style="width:200px;">
            <option value="side" <?php echo (($banner_type=="side")?"selected":"")?>>square (side)</option>
            <option value="bottom" <?php echo (($banner_type=="bottom")?"selected":"")?>>rectangle (bottom)</option>
            </select>
            </td>
            </tr>
            <tr><td>&nbsp;
            ...
            <td style="padding-left:20px;padding-top:8px;">
             <table cellpadding="0" cellspacing="0" border="0">
             <tr><td><select name="banner_position" style="width:200px;">
            <option value="1" <?php echo (($banner_position=="1")?"selected":"")?>>top left</option>
            <option value="2" <?php echo (($banner_position=="2")?"selected":"")?>>top right</option>
            <option value="3" <?php echo (($banner_position=="3")?"selected":"")?>>bottom left</option>
            <option value="4" <?php echo (($banner_position=="4")?"selected":"")?>>bottom right</option>
            </select></td></tr>
            <tr><td style="width:130px;text-align:left;font-style:italic;font-size:12px;color:#F00;">If banner type is square indicate here what's the position</td></tr></table>
            </td>
            </tr>
            ...
            <td style="padding-left:20px;padding-top:8px;">
            <input type="text" name="date_activate" id="datepicker" style="width:200px;" value="<?php echo $date_to_activate;?>"/>
            </td>
            </tr>
            <tr><td style="width:130px;text-align:right;padding-top:8px;">
            Date to end
            ...
            <td style="padding-left:20px;padding-top:8px;">
            <input type="text" name="date_end" id="datepicker2" style="width:200px;" value="<?php echo $date_to_end;?>"/>
            </td>
            </tr>
            <tr><td style="width:130px;text-align:right;padding-top:8px;">
            Banner name
            ...
            <td style="padding-left:20px;padding-top:8px;">
            <input type="text" name="banner_name" style="width:200px;" value="<?php echo $banner_name;?>"/>
            </td>
            </tr>
            <tr><td style="width:130px;text-align:right;padding-top:8px;">
            Upload banner
