...
class Dentist_Profile extends CI_Controller {
	...
	function save_dentist_info()
	{
		...
		$service_offered = array(
			$dental_emergency = $this->input->post('service')
			// json_encode($dental_emergency);
		);
		...
		$dentist_profile = array(
			...
			'dentist_services' => json_encode($service_offered),
		...
		// $dentist_id = $this->input->where('dentist-id');
	...
	function dentist_result($dentist_id)
	{
		$this->db->where('id', $dentist_id);
	...
	function separate_date($date = null) {
		...
		// echo $array_date['year'];
		// echo $array_date['month'];
		// echo $array_date['day'];
		return $array_date;
	}
}